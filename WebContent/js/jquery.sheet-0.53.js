...
var jS = jQuery.sheet = {
	...
	makeBarItemLeft: function(url) {//Works great!
		...
		var barLeft = jQuery('<div border="1px" id="' + jS.id.barLeft + '" />'); // MW .height('10000px');
		var heightFn;
	...
	makeBarItemTop: function(url) { //Works great!
		...
		var barTop = jQuery('<div id="' + jS.id.barTop + '" class="' + jS.cl.bar + '" />'); // MW .width('10000px');
			...
			if (jS.s.urlMenuJs && jS.s.urlMenuCss && jS.s.urlMetaData && jS.s.urlMenu) {
				...
				jQuery.getScript(jS.s.urlMenuJs, function() {
					...
					var menuObj = jQuery('<div />').load(jS.s.urlMenu, function() {
						...
						//Append Title
// MW						jS.obj.menu().find('#titleHolder').append('<span id="' + jS.id.title + '">' + sheetTitle + '</span>');

						jS.sheetSyncSize();
					});
				});
			} else {
// MW				controls.append('&nbsp;&nbsp;&nbsp;<span id="' + jS.id.title + '">' + sheetTitle + '</span>');
			}
...
// MW			.height(jS.obj.pane().height());

		jS.obj.barTopParent()
			.height(jS.s.colMargin);
// MW			.width(jS.obj.pane().width());
