/*
  ...
  // FIXME: can't keep plus sign in fields
  // FIXME: handle failed ajax requests
...
jQuery.fn.extend({
  sheet: function(settings) {
    settings = jQuery.extend({
      urlGet: false, // "documentation.html table:first",
      urlSave: false, // "save.html",
      title: 'Basic Sheet',
      readonly: true,
      urlBaseCss: false, // 'jquery.sheet.base.css',
      urlTheme: false, // "theme/ui.theme.css",
      urlMenu: false, // "menu.html",
      urlMenuJs: false, // "plugins/jquery.clickmenu.pack.js",
      urlMenuCss: false, // "plugins/clickmenu.css",
      urlScrollTo: false, // "plugins/jquery.scrollTo-1.4.0-min.js",
      loading: 'Loading Spreadsheet...',
      newColumnWidth: '60px',
//      ajaxSaveType: 'POST',
      buildSheet: false, // '10x20', // this can be slow
      calcOff: false,
      log: false,
      lockFormulas: false,
      field: this,  // this is the form field that hosts the data
      parent: null,
      colMargin: "17px",   // MW
      fnBefore: function() {},
      fnAfter: function() {},
    ...
    settings.width = jQuery(settings.field).width();
    settings.height = jQuery(settings.field).height();
    settings.readonly = jQuery(settings.field).attr('readonly');


    // hook into the parent form submit:
    jQuery(settings.field).parents("form").submit(function(event) {
      jS.saveSheet();
      return true;
      // event.preventDefault();
    });

    // for easy access to the settings
    jQuery.fn.sheet.settings = jS.s = settings;

    // for encoding/decoding see:
    //see. http://www.prodevtips.com/2008/10/21/jquery-plugin-html-decode-and-encode/
    //return;

    // read the data from the formField, this is some nice html structure from the textarea
    var tempSheet = jQuery(jS.s.field).val();
    // append a div for the visible sheet after the form field
    // the new div is the parent object used as container
    jQuery(jS.s.field).after('<table><tr><td><div id="' + jS.id.container + '" /></td></tr></table>');
    jS.s.parent = jQuery('#' + jS.id.container);
    // hide the field since we have a table for showing the data now
    jQuery(jS.s.field).hide();

    // an extra textarea for some logging
    if (jS.s.log) {
      jQuery(jS.s.parent).after('<textarea id="' + jS.id.log + '" />');
    }

    // this is the decoration for the sheet, column and row fields
    var sheetObj = jQuery(
      '<div id="' + jS.id.ui + '">'+
    ...
    //alert("tuning...");
    // custom load from the textarea data
    tunedSheet = tuneForUse(tempSheet, true);
    //alert("pane is: " + jQuery(sheetObj).find('#' + jS.id.pane) + " id was: " + jS.id.pane);
    jQuery(sheetObj).find('#' + jS.id.pane).html(tunedSheet);
    //alert("sheetObj is: " + jQuery(sheetObj).html());
...
//    }
 ...
 var jS = jQuery.sheet = {
  ...
  id: {//id = id's references
    container:      'jSheetContainer',
    sheet: 			'jSheet', //This can probably be just about any value as long as it's not a duplicated id
    ui:				'jSheetUI',
    barTop: 		'jSheetBarTop',
  ...
  sheetInit: function(sheetObj) {
    // alert("inserting into: " + jS.obj.parent().html());
  ...
  attr: {//I created this object so I could see, quickly, which attribute was most stable.
    ...
    height: function(obj) { return jQuery(obj).outerHeight(); /* jQuery(obj).outerHeight(); */ }
  },
  makeBarItemLeft: function() {
    ...
    jS.obj.sheet().find('tr').each(function(i) {
      // var newHeight = jS.attr.height(jQuery(this).find('tr:first'));
      // var newHeight = jS.attr.height(jQuery(this).find('td:first'));
      // var newHeight = jS.s.colMargin;
      var newHeight = jQuery(this).find('td:first').attr('height');
      if (!newHeight) {
        // try the row
        newHeight = jQuery(this).attr('height');
        if (!newHeight) {
        // set to default
        newHeight = jS.s.colMargin;
        }
      }
  ...
  saveSheet: function() {
    ...
    var sheet = jQuery('<div />').html(v).html();

    // alert("sheet: " + jQuery(sheet).html());

    // just put the stuff back into the form field:
    jQuery(jS.s.field).val(sheet);



    /*
    // need to replace "&" to a "%26"
    sheet = sheet.replace(/&/g,'%26');
 // need to replace "+" to a "%2B"
    sheet = sheet.replace(/&/g,'%2B');
  ...
  barHeightSync: function() {
    if (jQuery.browser.safari) {//Safari & Chrome have an odd issue with resize, this should fix it.
      jS.obj.barLeft().find('tr').each(function(i) {
        ...
        if (!h) {
          // FIXME. td may not have a height!
