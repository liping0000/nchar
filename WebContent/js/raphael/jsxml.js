...
function REXML(XML) {
	...
	function REXML_XMLElement(strType, strName, strAttributes, xmlParent, strText) {
		...
		function REXML_XMLElement_attribute(strAttributeName, newValue) {
			if (!this.attributes) {
				var reAttributes = new RegExp(" ([^= ]*)=","g"); // matches attribute names
				if (this.attributeString.match(reAttributes) && this.attributeString.match(reAttributes).length) {
					...
					if (!arrAttributes.length) arrAttributes = null;
						...
						// patched MW, we need to replace newlines
						var oneLineAttributeString = this.attributeString.replace(/[ \t\r\n\v\f]/g," ");
						arrAttributes[j] = new Array(
							(arrAttributes[j]+"").replace(/[= ]/g,""),
							ParseAttribute(oneLineAttributeString, (arrAttributes[j]+"").replace(/[= ]/g,""))
										);
					}
			...
			if (typeof(newValue) == "undefined") { // caller wants to get a value
			  if (this.attributes) {
			  ...
			  return "";
			} else {  // caller wants to set a value
		      if (this.attributes) {
				  for (var i=0; i<this.attributes.length; i++) {
					 if (this.attributes[i][0] == strAttributeName) {
					   this.attributes[i][1] = newValue;
					   return;
				     }
				  }
			  }
