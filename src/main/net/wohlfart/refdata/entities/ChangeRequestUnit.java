...
@NamedQueries({ 
    ...
    @NamedQuery(
        name = ChangeRequestUnit.FIND_BY_DEFAULT_NAME, 
        query = "from ChangeRequestUnit where defaultName = :defaultName"),
        
    @NamedQuery(
        name = ChangeRequestUnit.FIND_NEXT_INDEX_VALUE, 
        query = "select max(sortIndex) + 1 from ChangeRequestUnit "),
        
    @NamedQuery(
        name = ChangeRequestUnit.COUNT_QUERY, 
        query = "select count(*) from ChangeRequestUnit ")
})     
/*  @formatter:on */
        
@Entity
//not yet: @Audited
@Table(name = "CHREQ_UNIT")
public class ChangeRequestUnit implements ITranslateable, ISortable, Serializable {


    public static final String        FIND_BY_DEFAULT_NAME  = "ChangeRequestUnit.FIND_BY_DEFAULT_NAME";
    public static final String        FIND_NEXT_INDEX_VALUE = "ChangeRequestUnit.FIND_NEXT_INDEX_VALUE";
    public static final String        COUNT_QUERY           = "ChangeRequestUnit.COUNT_QUERY";
     ...
     */
    @Id
    @GenericGenerator(name = "sequenceGenerator", strategy = "org.hibernate.id.enhanced.TableGenerator", parameters = { @Parameter(name = "segment_value", value = "CHREQ_UNIT") })
    ...
    // registered
    @ManyToMany(mappedBy = "units" /* in the owning entity */)
    ...
    public void setupSortIndex(final Session hibernateSession) {
        final Object object = hibernateSession.getNamedQuery(ChangeRequestUnit.FIND_NEXT_INDEX_VALUE).uniqueResult();
        if (object == null) {
            sortIndex = 1;
        } else {
    ...
    public void setupMessageCode() {
        setMessageCode(CustomResourceLoader.CHREQ_UNIT_BUNDLE_NAME + TranslateableHome.MESSAGE_BUNDLE_ID_NUMBER_FORMAT.format(id));
    }
