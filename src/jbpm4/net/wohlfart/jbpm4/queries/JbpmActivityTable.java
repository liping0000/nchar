...
@Scope(CONVERSATION)
@Name(JbpmActivityTable.JBPM_ACTIVITY_TABLE)
public class JbpmActivityTable extends AbstractTableQuery<JbpmActivityTable.Row> {


    private final static Logger LOGGER              = LoggerFactory.getLogger(JbpmActivityTable.class);

    // needed to keep reference integrity in other components
    public static final String  JBPM_ACTIVITY_TABLE = "jbpmActivityTable";

    @Override
    public void setup() {
        setEjbql("select new " + JbpmActivityTable.Row.class.getName() + "( " + " e.dbid" + ", e.id" + ", e.name" + ", e.key" + ", e.state ) " + " from "
